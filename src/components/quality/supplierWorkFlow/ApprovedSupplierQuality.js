...
const ApprovedSupplierQuality = () => {
  ...
  const {
    ...
    size,
    resetFilter,
  } = usePaginate('approvedSupplierQuality', '/material-management/config/quality/supplier/search',{
  ...
  const getAllWorkFlow = async (status) => {
    try {
      setLoading(true)
      let { data } = await WorkflowActionService.getAllWorkflows(status, permissions.subModuleItems.QUALITY_SUPPLIER_PENDING_LIST);
    ...
    <CommonLayout>
      <ARMBreadCrumbs>
        <Breadcrumb separator="/">
          ...
          <Breadcrumb.Item>Approve Supplier</Breadcrumb.Item>
        </Breadcrumb>
      </ARMBreadCrumbs>

      <Permission permission='QUALITY_QUALITY_SUPPLIER_QUALITY_SUPPLIER_APPROVED_LIST_SEARCH' showFallback>
        <ARMCard title="Approve Supplier List">
          <Form
              form={form}
              onFinish={onFinish}
          >
                ...
                <ARMTable>
                  ...
                  {collection?.map((data) => (
                      <tr key={data.id}>
                        ...
                        {workFlowActions?.map((action) => (
                            <td key={action.id}>
                              {data.qualityApprovalStatuses[action.id]?.updatedByName || ''}
                            </td>
                        ))}
...
export default ApprovedSupplierQuality;
