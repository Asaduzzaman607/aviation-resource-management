...
const PendingShipmentProviderQuality = () => {
  const { Option } = Select;
  ...
  const {
    ...
    resetFilter,
    size
  } = usePaginate('pendingShipmentProviderQuality', '/material-management/config/quality/shipment_provider/search',{
  ...
  const handleStatus =  async (id) => {
    try {
      await shipmentService.toggleStatusQuality(
        id,
  ...
  const handleApprove = async (id, status,description = null) => {
    try {
      const {data} = await shipmentService.toggleApproveQuality(id, status,description = null)
...
<Permission permission='QUALITY_QUALITY_SHIPMENT_PROVIDER_QUALITY_SHIPMENT_PROVIDER_PENDING_LIST_SEARCH'showFallback>
      <ARMCard
        title={getLinkAndTitle(
          'Pending Shipment Provider LIST',
          '/material-management/shipment-provider/add',
          'blank',
          'QUALITY_QUALITY_SHIPMENT_PROVIDER_QUALITY_SHIPMENT_PROVIDER_PENDING_LIST_SAVE'
        )}
      >
        <Form form={form}
              onFinish={(values) =>
                        ...
                        <tr key={data.id}>
                          ...
                          <td>
                            <Space size='small'>
                             ...
                             <Permission permission='QUALITY_QUALITY_SHIPMENT_PROVIDER_QUALITY_SHIPMENT_PROVIDER_PENDING_LIST_EDIT'>
                               <Link
                                   to={`/quality/shipment-provider-edit/${data.id}`}
                               >
                                 <ARMButton disabled={!data.editable} type="primary" size="small" style={{
                              ...
                              <Permission permission='QUALITY_QUALITY_SHIPMENT_PROVIDER_QUALITY_SHIPMENT_PROVIDER_PENDING_LIST_DELETE'>
...
export default PendingShipmentProviderQuality;