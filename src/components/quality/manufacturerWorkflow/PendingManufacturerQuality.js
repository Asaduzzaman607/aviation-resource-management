...
import QualitySaveValidTillDateForm from "../supplierWorkFlow/QualitySaveValidTillDateForm";

const { Option } = Select;

const PendingManufacturerQuality = () => {
  ...
  const { isModalOpen, setIsModalOpen, data, handleViewDetails } = useViewDetails();
  ...
  const handleStatus = async (id) => {
    try {
      await QualityManufacturerService.handleStatus(
        id,
  ...
  const handleApprove = async (id, status) => {
    try {
      await QualityManufacturerService.handleApprove(id, status);
      refreshPagination();
      notifySuccess('Status changed successfully!');
    } catch (er) {
      notifyError(getErrorMessage(er));
  ...
  return (
    <CommonLayout>
      ...
      <Permission
        permission="QUALITY_QUALITY_MANUFACTURER_QUALITY_MANUFACTURER_PENDING_LIST_SEARCH"
        showFallback
      >
        <ARMCard
                      ...
                      <tr key={data.id}>
                        ...
                        <td>
                          <Space size="small">
                            ...
                            <Permission permission="QUALITY_QUALITY_MANUFACTURER_QUALITY_MANUFACTURER_PENDING_LIST_EDIT">
                              <Button
                                type="primary"
                                size="small"
                                style={{
                            ...
                            <Permission permission="QUALITY_QUALITY_MANUFACTURER_QUALITY_MANUFACTURER_PENDING_LIST_DELETE">
...
export default PendingManufacturerQuality;
