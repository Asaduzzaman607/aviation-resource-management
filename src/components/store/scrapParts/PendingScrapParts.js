...
const PendingScrapParts = () => {
  ...
  const {
    ...
    refreshPagination,
    size,
  } = usePaginate("pendingScrapParts", "/store/scraps/search");
  ...
  const handleStatus = async (id) => {
    try {
      await ScrapPartsService.toggleStatus(
        id,
  ...
  const handleApprove = async (id, status, description = null) => {
    try {
      await ScrapPartsService.toggleApprove(id, status, (description = null));
      refreshPagination();
      notifySuccess("Status changed successfully!");
    } catch (e) {
    ...
    <CommonLayout>
      <ARMBreadCrumbs>
        <Breadcrumb separator="/">
          ...
          <Breadcrumb.Item>Pending Scrap Parts</Breadcrumb.Item>
        </Breadcrumb>
      </ARMBreadCrumbs>
                    ...
                    {collection?.map((data) => (
                      <tr key={data.id}>
                        ...
                        <td>
                          <Space size="small">
                            <Link
                              to={`/store/pending-scrap-parts/view/${data.id}`}
                            >
                              <ARMButton
                                type="primary"
                                size="small"
                            ...
                            <Link to={`/store/edit-scrap-parts/${data.id}`}>
                              <ARMButton
                                type="primary"
                                size="small"
                                style={{
...
export default PendingScrapParts;
