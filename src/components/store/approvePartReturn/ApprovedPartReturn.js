...
const ApprovedPartReturn = () => {
  ...
  const {
    ...
    fetchData,
    size,
  } = usePaginate('approvedPartsReturn', '/store-return-parts/search', {
    type: 'APPROVED',
  });

  console.log('approved partreturn list', collection);

  const getAllWorkFlow = async () => {
    try {
      let { data } = await WorkflowActionService.workflowSearch(50, {
        isActive: true,
        subModuleItemId: subModules.STORE_RETURN_PARTS
      });
    ...
    <CommonLayout>
      <ARMBreadCrumbs>
        <Breadcrumb separator="/">
          ...
          <Breadcrumb.Item>&nbsp;Approved Parts Returns</Breadcrumb.Item>
        </Breadcrumb>
      </ARMBreadCrumbs>
      <Permission
        permission="STORE_PARTS_RETURN_APPROVED_PARTS_RETURN_SEARCH"
        showFallback
      >
        <ARMCard
          title={getLinkAndTitle('Approved Parts Return List', '/store')}
        >
          <Form
            form={form}
            onFinish={onFinish}
          >
            <Row gutter={20}>
              ...
              >
                ...
                {/* hidden field for paginating approve part return */}
                <Form.Item
                  name="type"
                  hidden
              ...
              <tbody>
                {collection?.map((data, index) => (
                  <tr key={data.id}>
                    ...
                    <td>
                      <Link
                        to={`/store/approved-parts-return/details/${data.id}`}
                      >
                        <ARMButton
                          type="primary"
                          size="small"
...
export default ApprovedPartReturn;
