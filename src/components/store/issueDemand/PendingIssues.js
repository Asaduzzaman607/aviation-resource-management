...
const PendingIssues = () => {
  ...
  const {
    ...
    refreshPagination,
    size,
  } = usePaginate('pendingIssueDemands', '/store/issues/search');

  console.log('pending issues: ', collection);
  ...
  const handleStatus = async (id) => {
    try {
      await IssueDemandService.toggleStatus(
        id,
  ...
  const handleApprove = async (id, params) => {
    try {
      await IssueDemandService.toggleApprove(id, params);
      refreshPagination();
      notifySuccess('Status changed successfully!');
    } catch (e) {
    ...
    <CommonLayout>
      <ARMBreadCrumbs>
        <Breadcrumb separator="/">
          ...
          <Breadcrumb.Item>{t('Pending Issue')}</Breadcrumb.Item>
        </Breadcrumb>
      </ARMBreadCrumbs>
      <Permission
        permission="STORE_PARTS_ISSUE_PENDING_ISSUE_SEARCH"
        showFallback
      >
        <ARMCard
          title={getLinkAndTitle(
            ...
            '/store/issue-demand',
            true,
            'STORE_PARTS_ISSUE_PENDING_ISSUE_SAVE'
          )}
        >
          <Form
            form={form}
            ...
            <>
              <ResponsiveTable>
                <ARMTable>
                  <thead>
                    <tr>
                      <th rowSpan={2}>{t('Voucher No')}</th>
                      <th colSpan={workFlowActions.length}>{t('Approvals')}</th>
                      <th rowSpan={2}>{t('Status')}</th>
                      {isActive === Status.ACTIVE && (
                        <th rowSpan={2}>{t('Approve')}</th>
                      )}
                    ...
                    {collection?.map((data, index) => (
                      <tr key={data.id}>
                        ...
                        <td>
                          <Space size="small">
                            <Link
                              to={`/store/pending-issues/details/${data.id}`}
                            >
                              <ARMButton
                                type="primary"
                                size="small"
                            ...
                            <Permission permission="STORE_PARTS_ISSUE_PENDING_ISSUE_EDIT">
                              <Link to={`/store/issue-demand/edit/${data.id}`}>
                                <ARMButton
                            ...
                            <Permission permission="STORE_PARTS_ISSUE_PENDING_ISSUE_DELETE">
