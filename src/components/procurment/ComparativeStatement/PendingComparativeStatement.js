...
const PendingComparativeStatement = () => {
  ...
  const getAllWorkFlow = async (status) => {
    try {
      ...
      let { data } = await WorkflowActionService.getAllWorkflows(
        status,
        subModules.COMPARATIVE_STATEMENTS_MATERIAL_MANAGEMENT
      );
  ...
  const handleStatus = async (id) => {
    try {
      await CSService.pendingToggleStatus(id, isActive !== Status.ACTIVE ? Status.ACTIVE : Status.INACTIVE)
  ...
  const handleApprove = async (id, status, description = null) => {
    try {
      await CSService.toggleApprove(id, status, (description = null));
      refreshPagination();
      notifySuccess("Status changed successfully!");
    } catch (e) {
    ...
    <CommonLayout>
      <ARMBreadCrumbs>
        <Breadcrumb separator="/">
          ...
          <Breadcrumb.Item>Pending CS</Breadcrumb.Item>
        </Breadcrumb>
      </ARMBreadCrumbs>
      <Permission
        permission="MATERIAL_MANAGEMENT_MATERIAL_MANAGEMENT_COMPARATIVE_STATEMENT_MATERIAL_MANAGEMENT_PENDING_CS_SEARCH"
        showFallback
      >
        <ARMCard
          title={getLinkAndTitle(
            ...
            true,
            "MATERIAL_MANAGEMENT_MATERIAL_MANAGEMENT_COMPARATIVE_STATEMENT_MATERIAL_MANAGEMENT_PENDING_CS_SAVE",
            "MATERIAL_MANAGEMENT_MATERIAL_MANAGEMENT_COMPARATIVE_STATEMENT_MATERIAL_MANAGEMENT_PENDING_CS_EDIT"
          )}
        >
          <Form
            form={form}
                      ...
                      <tr key={data.id}>
                        ...
                        <td>
                          <Space size="small">
                            ...
                            <Permission permission="MATERIAL_MANAGEMENT_MATERIAL_MANAGEMENT_COMPARATIVE_STATEMENT_MATERIAL_MANAGEMENT_PENDING_CS_SEARCH">
                            <Link
...
export default PendingComparativeStatement;
