...
const CommentSection = ({ partDetailId }) => {
  ...
  const getDisposals = async (partDetailId) => {
    try {
      const { data } = await API.get(
        `/logistic/comparative-statements/audit-management/disposal/${partDetailId}`
      );
  ...
  const getSingleComment = async (id) => {
    try {
      setLoading(true);
      const { data } = await API.get(
        `/logistic/comparative-statements/audit-management/single/disposal/${id}`
      );
      setCommentId(id);
  ...
  const onFinish = async (values) => {
    try {
      ...
      if (commentId) {
        ...
        const files = await handleFilesUpload('Logistic', selectedFile);

        await API.put(
          `/logistic/comparative-statements/audit-management/disposal/${commentId}`,
        ...
        await API.post(
          `/logistic/comparative-statements/audit-management/disposal`,
          {
            ...values,
            attachments: files,
