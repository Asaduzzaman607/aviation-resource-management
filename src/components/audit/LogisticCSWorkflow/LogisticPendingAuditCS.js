...
const LogisticPendingAuditCS = () => {
  ...
  const handleStatus = async (id) => {
    try {
      await API.patch(
        `/logistic/comparative-statements/audit-management/${id}?active=${
  ...
  const handleApprove = async (id, status, description = null) => {
    try {
      await LogisticCSService.toggleApproveAudit(
        id,
        status,
        (description = null)
      );
  ...
  return (
    <CommonLayout>
      ...
      <Permission
        permission="LOGISTIC_LOGISTIC_COMPARATIVE_STATEMENT_LOGISTIC_AUDIT_PENDING_CS_SEARCH"
        showFallback
      >
        <ARMCard
          title={getLinkAndTitle('Pending Logistic Audit CS LIST', '/audit')}
        >
          <Form
            form={form}
            onFinish={(values) =>
                  ...
                  <tbody>
                    {collection?.map((data) => (
                      <tr key={data.id}>
                        ...
                        <td>
                          <Space size="small">
                            <Permission permission="LOGISTIC_LOGISTIC_COMPARATIVE_STATEMENT_LOGISTIC_AUDIT_PENDING_CS_SEARCH">
                              
                              <Link to={`/audit/logistic/pending-audit-cs/detail/${data.id}`} state={{ approvedOrPending: "pending" }}>
                              <ARMButton
                                type="primary"
                                size="small"
                                style={{
                            ...
                            <Permission permission="LOGISTIC_LOGISTIC_COMPARATIVE_STATEMENT_LOGISTIC_AUDIT_PENDING_CS_DELETE">
                        ...
                        <td>
                          <Link
                              to={`/audit/logistic/pending-audit-cs-disposal/add/${data.id}`}
                            >
                              <ARMButton
...
export default LogisticPendingAuditCS;
