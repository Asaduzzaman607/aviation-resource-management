...
const PendingAuditCS = () => {
  ...
  const {
    ...
    'pendingAuditCS',
    '/procurement/comparative-statements/audit-management/search',
    { rfqType: 'PROCUREMENT' }
  );

  console.log('pending audit cs list', collection);
  ...
  const getAllWorkFlow = async (status) => {
    try {
      ...
      let { data } = await WorkflowActionService.getAllWorkflows(
        status,
        subModules.COMPARATIVE_STATEMENTS_AUDIT_MANAGEMENT
      );
  ...
  const handleStatus = async (id) => {
    try {
      await API.patch(
        `/procurement/comparative-statements/audit-management/${id}?active=${
  ...
  const handleApprove = async (id, status, description = null) => {
    try {
      await CSService.toggleApproveAudit(id, status, (description = null));
      refreshPagination();
      notifySuccess('Status changed successfully!');
    } catch (e) {
    ...
    <CommonLayout>
      <ARMBreadCrumbs>
        <Breadcrumb separator="/">
          ...
          <Breadcrumb.Item>Pending Audit CS</Breadcrumb.Item>
        </Breadcrumb>
      </ARMBreadCrumbs>

      <Permission
        permission="MATERIAL_MANAGEMENT_MATERIAL_MANAGEMENT_COMPARATIVE_STATEMENT_MATERIAL_MANAGEMENT_AUDIT_PENDING_CS_SEARCH"
        showFallback
      >
        <ARMCard title={getLinkAndTitle('Pending Audit CS LIST', '/audit')}>
          <Form
            form={form}
            onFinish={(values) =>
              isActive === Status.REJECTED
                  ...
                  <tbody>
                    {collection?.map((data) => (
                      <tr key={data.id}>
                        ...
                        <td>
                          <Space size="small">
                            <Permission permission="MATERIAL_MANAGEMENT_MATERIAL_MANAGEMENT_COMPARATIVE_STATEMENT_MATERIAL_MANAGEMENT_AUDIT_PENDING_CS_SEARCH">
                              <Link to={`/audit/pending-cs-details/${data.id}`}>
                                <ARMButton
                                  type="primary"
                                  size="small"
                                  style={{
                            ...
                            <Permission permission="MATERIAL_MANAGEMENT_MATERIAL_MANAGEMENT_COMPARATIVE_STATEMENT_MATERIAL_MANAGEMENT_AUDIT_PENDING_CS_DELETE">
                        ...
                        <td>
                          <Space size="small">
                            <Link
                              to={`/audit/pending-audit-cs-disposal/add/${data.id}`}
                            >
                              <ARMButton
