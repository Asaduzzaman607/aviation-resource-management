    ...
    <CommonLayout>
      <ARMBreadCrumbs>
        <Breadcrumb separator="/">
          ...
          <Breadcrumb.Item>Work Package Certification Record</Breadcrumb.Item>
        </Breadcrumb>
      </ARMBreadCrumbs>
      <Permission permission="PLANNING_CHECK_WORK_PACKAGE_CERTIFICATION_RECORD_SEARCH" showFallback>
      <ARMCard title={getLinkAndTitle('Work Package Certification Record', "/planning/work-package-certification-record/add", true,"PLANNING_CHECK_WORK_PACKAGE_CERTIFICATION_RECORD_SAVE")}>
        <ARMForm initialValues={{aircraftId : null, pageSize: 10 , packageType : 1}} onFinish={fetchData} form={form}>
          <Row gutter={20}>
        ...
        <Row className="table-responsive">
          <ResponsiveTable>
            <ARMTable>
              ...
              <tbody>
              {collection?.filter(({packageType})=> packageType === 'WORK_PACKAGE_CERTIFICATE_RECORD')?.map((workPackage, index) => (
                <tr key={index}>
                  ...
                  <td>
                    <Space size="small">
                      ...
                      {isActive? <Link to={`edit/${workPackage?.workPackageId}`}>
                        <Permission permission="PLANNING_CHECK_WORK_PACKAGE_CERTIFICATION_RECORD_EDIT">
                          <EditButton/>
                        </Permission>
                      </Link> : null}
                      
                      <Permission permission="PLANNING_CHECK_WORK_PACKAGE_CERTIFICATION_RECORD_DELETE">
...
export default WorkPackageCertificationRecordList;
