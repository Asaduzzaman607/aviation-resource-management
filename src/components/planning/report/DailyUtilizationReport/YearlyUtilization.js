...
export default function YearlyUtilizationRecord() {
    ...
    const handleMonthlyUtilizationSearch = useCallback(
        async (values) => {
            ...
            if (daysDiff > 1825) {
                notifyWarning("Please select start date and end date within 5 years !");
                return;
            }
            setStartDate(moment(fromDate).format("YYYY"));
            const lastDate = moment(toDate).endOf("year");
            setEndDate(lastDate);
    ...
    const {onDownload} = useDownloadExcel({
        ...
        filename: 'Yearly Utilization Report',
        sheet: 'yearlyUtilizationReport'
    })
        ...
        <CommonLayout>
            <ARMBreadCrumbs>
                <Breadcrumb separator="/">
                    ...
                    <Breadcrumb.Item>Yearly utilization record</Breadcrumb.Item>
                </Breadcrumb>
            </ARMBreadCrumbs>
            <Permission permission="PLANNING_AIRCRAFT_TECHNICAL_LOG_YEARLY_UTILIZATION_RECORD_SEARCH" showFallback>
                <ARMCard
                    title={
                        <Row justify="space-between">
                            <Col>YEARLY UTILIZATION RECORD</Col>
                            <Col>
                                <Space>
                                ...
                                <Row className="table-responsive">
                                    <ResponsiveTable className="first">
                                        <ARMReportTable className="table">
                                            ...
                                            <tr>
                                                <th className="none" colSpan={2}></th>
                                                <th>MSN : {aircraftApuDetails?.airframeSerial ? aircraftApuDetails?.airframeSerial : 'N/A'}</th>
