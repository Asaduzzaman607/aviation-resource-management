...
const TITLE = "Removal Engine LLP's Status";
...
export default function EngineLLPsStatus() {
  ...
  const getACBuild = async () => {
    const { data } = await API.get(`aircraft-build/find-inactive-engine-by-aircraft/${aircraftId}`)
    setAcBuilds(data)
  }
  ...
  const getEngineLLP = async () => {
    ...
    try{
      const { data } = await API.post(`engine/inactivate-engine-llp-status-report`, { aircraftBuildId })
      setData(data)
  ...
  return (
    <CommonLayout>
      ...
      <Permission permission="PLANNING_ENGINE_PROPELLER_LANDING_GEAR_REMOVED_ENGINE_LLP_STATUS_SEARCH" showFallback>
      <ARMCard
        title={
          ...
          <Row gutter={20}>
            ...
            <Col xs={24} md={6}>
              ...
              >
                ...
                <Select
                  placeholder="Select Engine SerialNo"
                >
                  {
                    acBuilds.map((acBuild: { aircraftBuildId: any, serialNo: any; }) => <Select.Option value={acBuild?.aircraftBuildId} key={acBuild?.aircraftBuildId}>{acBuild?.serialNo}</Select.Option>)
                  }
                </Select>
              </Form.Item>
            </Col>
