...
const LogisticFinalApprovedCS = () => {
  ...
  const { form, collection, page, totalElements, paginate, fetchData, size } =
    usePaginate(
      'finalLogisticApprovedCS',
      '/logistic/comparative-statements/final-management/search',
      { rfqType: 'LOGISTIC', type: 'APPROVED' }
    );

  console.log('finalApprovedCS: ', collection);
    ...
    <CommonLayout>
      <ARMBreadCrumbs>
        <Breadcrumb separator="/">
          ...
          <Breadcrumb.Item>Approve Logistic CS</Breadcrumb.Item>
        </Breadcrumb>
      </ARMBreadCrumbs>

      <Permission
        permission="LOGISTIC_LOGISTIC_COMPARATIVE_STATEMENT_LOGISTIC_FINAL_APPROVED_CS_SAVE"
        showFallback
      >
        <ARMCard
          title={getLinkAndTitle(
            'Final Approved Logistic CS LIST',
            '/logistic',
            false,
            'LOGISTIC_LOGISTIC_COMPARATIVE_STATEMENT_LOGISTIC_FINAL_APPROVED_CS_SAVE'
          )}
        >
          <Form
            form={form}
                  ...
                  <tbody>
                    {collection?.map((data) => (
                      <tr key={data.id}>
                        ...
                        <td>
                          <Permission permission="LOGISTIC_LOGISTIC_COMPARATIVE_STATEMENT_LOGISTIC_FINAL_APPROVED_CS_SAVE">
                            <Link
                              to={`/logistic/final-approved-comparative-statement/detail/${data.id}`}
                            >
                              <ARMButton
                                type="primary"
                                size="small"
                        ...
                        <td>
                          <Permission permission="LOGISTIC_LOGISTIC_COMPARATIVE_STATEMENT_LOGISTIC_FINAL_APPROVED_CS_EDIT">
                            <Link
                              to={`/logistic/comparative-statement/disposal/edit/${data.id}`}
                            >
                              <ARMButton
                                type="primary"
                                size="small"
...
export default LogisticFinalApprovedCS;
