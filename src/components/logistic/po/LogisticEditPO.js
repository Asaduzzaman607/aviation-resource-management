...
const LogisticEditPO = () => {
  ...
  const getTableData1 = async () => {
    try {
      const { data } = await API.get(`/logistic/part-orders/${id}`);
      form.setFieldsValue({
        tac: data.tac,
        remark: data.remark,
  ...
  const onFinish = async (values) => {
    ...
    try {
      await API.put(`/logistic/part-orders/${id}`, modifiedValue);
    ...
    <CommonLayout>
      ...
      <ARMCard
        title={getLinkAndTitle(
          'logistic purchase orders',
          '/logistic/purchase-order'
        )}
      >
        <ARMForm
          {...layout}
