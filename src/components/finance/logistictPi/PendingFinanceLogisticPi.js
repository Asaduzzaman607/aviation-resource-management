...
const PendingFinanceLogisticPi = () => {
  ...
  const handleStatus = async (id) => {
    try {
      await LogisticPIService.toggleStatusFinance(
        id,
  ...
  const handleApprove = async (id, status, description = null) => {
    try {
      await LogisticPIService.toggleApproveFinance(
        id,
        status,
        (description = null)
      );
  ...
  return (
    <CommonLayout>
      ...
      <Permission
        permission="FINANCE_LOGISTIC_PARTS_INVOICE_FINANCE_LOGISTIC_PENDING_PARTS_INVOICE_FINANCE_SEARCH"
        showFallback
      >
        <ARMCard
          title={getLinkAndTitle(
            ...
            'blank',
            'FINANCE_LOGISTIC_PARTS_INVOICE_FINANCE_LOGISTIC_PENDING_PARTS_INVOICE_FINANCE_SAVE'
          )}
        >
          <Form
            form={form}
                  ...
                  <tbody>
                    {collection?.map((data) => (
                      <tr key={data.id}>
                        ...
                        <td>
                          <Space size="small">
                            <Permission permission="FINANCE_LOGISTIC_PARTS_INVOICE_FINANCE_LOGISTIC_PENDING_PARTS_INVOICE_FINANCE_SEARCH">
                              <Link
                                to={`/finance/logistic/pending-purchase-invoice/details/${data.id}`}
                              >
                                <ARMButton
                                  type="primary"
                                  size="small"
                            ...
                            <Permission permission="FINANCE_LOGISTIC_PARTS_INVOICE_FINANCE_LOGISTIC_PENDING_PARTS_INVOICE_FINANCE_DELETE">
...
export default PendingFinanceLogisticPi;
